...
import { TRIAL_DAYS, TRIAL_MAX_ADMINS, TRIAL_MAX_LAWYERS } from '../subscription/plans';
    ...
    const result = await this.system.$transaction(async (tx) => {
      ...
      const tenant = await tx.tenant.create({
        data: {
          ...
          // Prueba gratis de 15 días con todo abierto y plazas generosas para evaluar en equipo.
          // Al expirar sin suscripción → muro (SubscriptionGuard). Stripe self-service por plaza luego.
          subscriptionStatus: 'TRIALING',
          trialEndsAt: new Date(Date.now() + TRIAL_DAYS * 24 * 60 * 60 * 1000),
          maxAdmins: TRIAL_MAX_ADMINS,
          maxLawyers: TRIAL_MAX_LAWYERS,
          // `seats` (plazas contratadas) queda en 0 durante la prueba; se fija al suscribirse.
        },
      });
