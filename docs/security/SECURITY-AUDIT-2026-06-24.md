...
## 9. Estado de remediación — pasada 2026-06-24 (rama `security/hardening-2026-06-24`)

Corregido y verificado en local (typecheck API+web+compliance ✅, lint 0 errores, fiscal-conformance **7/7** incl. cadena multi-registro, unit API 19/19). La verificación de BD (RLS/privilegios/migración) corre en CI real (Postgres como `legalflow_app`).

| ID                                               | Estado                       | Fix aplicado                                                                                                                                                                                                                                                                    |
| ------------------------------------------------ | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **D8-001 / D10-001**                             | ✅                           | Migración `20260624120000_fiscal_audit_immutability`: privilegios de columna en `Invoice` (rol app pierde UPDATE de columnas fiscales + DELETE; conserva solo ciclo de vida); `AuditLog` append-only (REVOKE UPDATE/DELETE al rol app). Rol de sistema conserva todo (cascada). |
| **D8-002**                                       | ✅                           | `InvoiceSequence` (contador monótono por `tenant:serie:año`) sustituye a `COUNT(*)+1`; año derivado de `issueDate`.                                                                                                                                                             |
| **D8-003**                                       | ✅                           | Génesis constante (64 ceros) en `ledger.service` + orden determinista `(createdAt, id)`.                                                                                                                                                                                        |
| **D8-004**                                       | ✅                           | Tabla `FiscalEvent` append-only encadenada (RRSIF) + `appendFiscalEvent` en cada emisión + `verifyFiscalChain`.                                                                                                                                                                 |
| **D8-006**                                       | ✅                           | Test de conformidad multi-registro (cadena continua, determinismo, detección de manipulación de importe).                                                                                                                                                                       |
| **D8-005**                                       | ⏳ Diferido (fase cert DGII) | eNCF de rango autorizado + huella sobre XML firmado + XAdES-BES: requieren el certificado/rango reales del owner.                                                                                                                                                               |
| **D2-002**                                       | ✅                           | Login Microsoft exige `email_verified`; sin fallback a `upn`/`preferred_username`. **Owner:** confirmar restricción de tenant de la app MS.                                                                                                                                     |
| **D2-001**                                       | ✅ parcial                   | `state` ligado a cookie nonce + PKCE; binding estricto cuando la cookie está presente (prod siempre la fija). Endurecer a obligatorio queda al ajustar el e2e.                                                                                                                  |
| **D2-005 / D2-006 / D2-004**                     | ✅                           | Incremento de fallos MFA atómico; lockout en login multi-tenant; `mfaToken` single-use por `jti` (in-memory, por instancia).                                                                                                                                                    |
| **D-PLAT (D1-001/D2-003/D3-001/D3-004/D10-002)** | ✅ código                    | `PLATFORM_JWT_SECRET` fatal en prod + `aud=platform`; TTL 1 h; auditoría de toda acción de plataforma (actor+IP). MFA del super-admin y BFF httpOnly/denylist: diferidos. **Owner:** fijar `PLATFORM_JWT_SECRET`/`PLATFORM_ADMIN_PASSWORD`.                                     |
| **D4-001**                                       | ✅                           | Webhook Signaturit resuelve `tenantId` desde la fila por `externalId` (no del payload). Verificación de firma del proveedor + integridad eIDAS del documento: al activar firma real.                                                                                            |
| **D6-001**                                       | ✅                           | Keyring `DATA_ENCRYPTION_KEY` + `DATA_ENCRYPTION_KEY_RETIRED` en storage/mfa/dgii/google/microsoft → **la rotación de la clave maestra (C1) ya es posible** sin romper el descifrado.                                                                                           |
| **D5-001 / D10-005**                             | ✅                           | Cuota por tokens (`AI_DAILY_TOKEN_LIMIT`) + evento `ai.quota_exhausted`.                                                                                                                                                                                                        |
| **D10-003**                                      | ✅                           | Auditoría de descargas internas de documentos (`document.downloaded`).                                                                                                                                                                                                          |
| **D7-001**                                       | ✅                           | `assertUploadSafe` en adjuntos de correo entrante (`createSystemDocument`).                                                                                                                                                                                                     |
| **D1-002 / D4-003 / D7-002 / D7-003 / D9-001**   | ✅                           | FK scoping en `closing`; `encodeURIComponent` en `searchSites`; DTOs en data-room; rotación de token de intake; Actions de 1ª parte ancladas a SHA.                                                                                                                             |
| **D5-002**                                       | ⏳ Diferido                  | Throttle de IA por tenant (hoy por IP): subsumido en la práctica por el presupuesto de tokens por tenant.                                                                                                                                                                       |
| **D10-004**                                      | ⏳ Diferido                  | Auditoría del ciclo e-CF/DGII: ligado a la fase de transmisión real.                                                                                                                                                                                                            |
| **D10-006 / D9-004 / D4-002**                    | ⏳ Diferido (infra)          | Sink WORM externo; escaneo de imagen (Trivy); dedupe de webhook por `event.id`.                                                                                                                                                                                                 |
| **D6-P1 / D6-P2 / D6-P3 / D6-P4**                | ⏳ Owner/legal               | DPA + registro de subencargados; gating/consentimiento de IA con salvaguarda de transferencia; residencia ES↔RD; export de acceso/portabilidad. Bloquean venta con datos reales; no son "bugs" de código.                                                                       |

**Acciones de OWNER pendientes (no código):** rotar **todos** los secretos de C1 — ahora posible para `DATA_ENCRYPTION_KEY` con el keyring (poner la actual en `DATA_ENCRYPTION_KEY_RETIRED`, la nueva en `DATA_ENCRYPTION_KEY`, desplegar, re-cifrar, retirar la vieja); fijar `PLATFORM_JWT_SECRET` (≠ `JWT_ACCESS_SECRET`) y un `PLATFORM_ADMIN_PASSWORD` fuerte; `HIBP_ENABLED=true`; confirmar restricción de tenant de la app Microsoft.
